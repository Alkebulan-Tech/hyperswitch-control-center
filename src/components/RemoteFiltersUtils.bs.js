...
function getFinalDict(
  ...
  if (dict !== undefined) {
    ...
    var allowedDefaultKeys = Core__Option.isSome(dict$1["sourceObject"])
      ? Object.keys(dict$1)
      : defaultKeysAllowed;
  ...
  Object.entries(unflattenDict).forEach(function (entry) {
    ...
    filterDict[key] = Curry._1(parser, val);
    ...
    if (match !== undefined) {
      ...
      if (typeof stringValueArr === "number") {
        val$1 = val;
      } else {
        switch (stringValueArr.TAG | 0) {
          case /* String */ 1:
    ...
    match[0].forEach(function (key, idx) {
      initialFilters.forEach(function (filter) {
        ...
        field.inputNames.forEach(function (name) {
          ...
          if (!value.includes("[")) {
            res.contents =
              localFilter !== undefined
                ? Curry._2(localFilter, res.contents, value)
                : res.contents;
            return;
          }
          ...
          res.contents =
            localFilter !== undefined
              ? Curry._2(localFilter, res.contents, jsonarr)
              : res.contents;
        });
      });
      options.forEach(function (option) {
...
function generateUrlFromDict(dict, options, tableName) {
  return Belt_Array.keepMap(Object.entries(dict), function (entry) {
    ...
    if (requiredOption !== undefined) {
      var finalVal = Curry._1(requiredOption.parser, val);
      dict[key] = finalVal;
    } else {
      dict[key] = val;
    }
    var finalKey =
      tableName !== undefined ? tableName.concat("-" + key + "") : key;
...
function applyFilters(
  ...
  if (originalDict !== undefined) {
    Object.entries(Caml_option.valFromOption(originalDict)).forEach(
      function (entry) {
  ...
  if (ignoreUrlUpdate) {
    if (setLocalSearchFilters !== undefined) {
      return Curry._1(setLocalSearchFilters, function (param) {
