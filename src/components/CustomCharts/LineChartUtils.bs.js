...
function legendTypeBasedOnMetric(metric_type) {
  switch (metric_type) {
    case /* Volume */ 1:
      return [/* Overall */ 1, /* Average */ 2];
    case /* Rate */ 2:
      return [/* Current */ 3, /* Average */ 2];
    case /* Amount */ 3:
      return [/* Average */ 2, /* Overall */ 1];
    default:
      return [/* Current */ 3, /* Average */ 2];
  }
}
...
function timeSeriesDataMaker(
  ...
  data.map(function (item) {
    ...
    var groupByName = LogicUtils.getString(
      dict,
      groupKey,
...
function barChartDataMaker(yAxis, rawData, activeTab) {
  var value = Belt_Array.keepMap(rawData, function (item) {
    var dict = LogicUtils.getDictFromJsonObject(item);
    var selectedSegmentVal = LogicUtils.getString(
      dict,
      activeTab,
      JSON.stringify(Core__Option.getOr(dict[activeTab], "")),
    );
...
function getTooltipHTML(metrics, data, onCursorName) {
  ...
  var secondry_metrix_val =
    secondryMetrics !== undefined
      ...
      : "";
  var highlight =
    onCursorName === name ? "font-weight:900;font-size:13px;" : "";
  return (
    "<tr>\n      <td><span style='color:" +
    data[1] +
    "; " +
...
function tooltipFormatter(metrics, xAxisMapInfo, groupKey) {
  return function () {
    ...
    var htmlStr = dataArr
      .map(function (data) {
        return getTooltipHTML(metrics, data, onCursorName);
      })
      .join("");
...
function legendItemStyle(theme, legendFontFamilyClass, legendFontSizeClass) {
  if (theme) {
    return {
      color: "#c7cad0",
      cursor: "pointer",
...
function legendHiddenStyle(theme, legendFontFamilyClass, legendFontSizeClass) {
  if (theme) {
    return {
      color: "#c7cad020",
      cursor: "pointer",
...
function getGranularity(startTime, endTime) {
  ...
  if (diff < 60 * 6) {
    ...
    return ["G_ONEHOUR", "G_THIRTYMIN", "G_FIFTEENMIN"];
  } else if (diff < 60 * 24 * 7) {
    return ["G_ONEDAY", "G_ONEHOUR"];
  } else {
    return ["G_ONEDAY"];
  }
}
...
function chartDataMaker(filterNullOpt, rawData, groupKey, metric) {
  ...
  return rawData
    .filter(function (dataPoint) {
      ...
      return LogicUtils.getString(dataPointDict, groupKey, "") !== "other";
    })
