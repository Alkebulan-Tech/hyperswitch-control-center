...
function getDictFromJsonObject(json) {
  ...
  if (dict !== undefined) {
    return Caml_option.valFromOption(dict);
  } else {
    return {};
  }
}
...
function getStrArrayFromDict(dict, key, $$default) {
  return Core__Option.getOr(
    Core__Option.flatMap(dict[key], getOptionStrArrayFromJson),
    $$default,
  );
}

function getOptionStrArrayFromDict(dict, key) {
  return Core__Option.flatMap(dict[key], getOptionStrArrayFromJson);
}
...
var getOptionFloatFromString = Core__Float.fromString;
...
function getObj(dict, key, $$default) {
  return Core__Option.getOr(
    Core__Option.flatMap(dict[key], Core__JSON.Decode.object),
    $$default,
  );
}
...
function latencyShortNum(labelValue, includeMilliseconds, param) {
  ...
  var millisec_disp =
    ...
    labelValue > 0.0
      ...
      : "";
  var sec_disp =
    seconds > 0 ? "" + String(seconds) + "" + millisec_disp + "S " : "";
  if (days > 0) {
...
function getDefaultNumberFormat(param) {
  return /* USD */ 1;
}

function indianShortNum(labelValue) {
  return shortNum(labelValue, /* USD */ 1, undefined, undefined);
}
...
function convertNewLineSaperatedDataToArrayOfJson(text) {
  return text
    ...
    .map(function (item) {
      return Core__Option.getOr(safeParseOpt(item), null);
    });
}
...
function getJsonFromStr(data) {
  if (data.length > 0) {
    return JSON.stringify(
      Core__Option.getOr(safeParseOpt(data), null),
      null,
      2,
    );
  } else {
