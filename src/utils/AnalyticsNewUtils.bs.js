...
function calculateHistoricTime(
  ...
  var timeZone = timeZoneOpt !== undefined ? timeZoneOpt : /* UTC */ 0;
  var toUtc = timeZone
    ? function (val) {
        return val;
      }
  ...
  var startDateTime = Curry._1(
    toUtc,
    new Date(DateTimeUtils.parseAsFloat(startTime)),
  );
  var startTimeDayJs = DayJs.getDayJsForJsDate(startDateTime);
  var endDateTime = Curry._1(
    toUtc,
    new Date(DateTimeUtils.parseAsFloat(endTime)),
  );
...
function makeFilters(filters, cardinalityArr) {
  ...
  return Caml_option.some(complexFilterDict);
}
...
function getFilterBody(
  ...
  if (customFilterValue !== undefined) {
    ...
    if (filterValueArr.length === 1) {
      ...
      customFilterBuild = Caml_option.some(complexFilterDict);
    } else {
  ...
  if (filterValueFromUrl !== undefined) {
    var value$1 = Caml_option.valFromOption(filterValueFromUrl);
    if (customFilterBuild !== undefined) {
      var customFilter = Caml_option.valFromOption(customFilterBuild);
      var formattedFilters = makeFilters(value$1, cardinalityArrFilter);
      filterValue =
        formattedFilters !== undefined
          ? Object.fromEntries([
              [
                "and",
                Object.fromEntries([
                  ["left", Caml_option.valFromOption(formattedFilters)],
                  ["right", customFilter],
                ]),
              ],
            ])
      ...
      var formattedFilters$1 = makeFilters(value$1, cardinalityArrFilter);
      filterValue =
        formattedFilters$1 !== undefined
          ? Caml_option.valFromOption(formattedFilters$1)
          : {};
    }
  } else {
    filterValue =
      customFilterBuild !== undefined
        ? Caml_option.valFromOption(customFilterBuild)
        : {};
  }
  if (jsonFormattedFilter === undefined) {
    return filterValue;
  }
  var jsonFormattedFilter$1 = Caml_option.valFromOption(jsonFormattedFilter);
  if (Object.entries(filterValue).length > 0) {
    return Object.fromEntries([
      [
        ...
        Object.fromEntries([
          ["left", filterValue],
          ["right", jsonFormattedFilter$1],
        ]),
      ],
    ]);
  } else {
    return Core__Option.getOr(
      Core__JSON.Decode.object(jsonFormattedFilter$1),
      {},
    );
  }
}
...
function apiBodyMaker(
  ...
  var cardinalitySortDims =
    cardinalitySortDimsOpt !== undefined
      ? cardinalitySortDimsOpt
      : "total_volume";
  var timeZone = timeZoneOpt !== undefined ? timeZoneOpt : /* IST */ 1;
  ...
  if (granularityConfig !== undefined) {
    ...
    var granularity = {};
    granularityDimension["timeZone"] = timeZone ? "Asia/Kolkata" : "UTC";
