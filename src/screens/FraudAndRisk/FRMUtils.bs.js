...
function generateInitialValuesDict(selectedFRMInfo, isLiveMode, param) {
  var frmAccountDetailsDict = LogicUtils.getJsonFromArrayOfJson([
    ["auth_type", FRMInfo.getFRMAuthType(selectedFRMInfo.name)],
  ]);
  return Object.fromEntries([
    ["connector_name", FRMInfo.getFRMNameString(selectedFRMInfo.name)],
...
function updateConfigDict(configDict, connectorName, paymentMethodsDict) {
  ...
  if (prevPaymentMethodsDict !== undefined) {
    var prevPaymentMethodsDict$1 = Caml_option.valFromOption(
      prevPaymentMethodsDict,
    );
    Object.keys(paymentMethodsDict).forEach(function (pmName) {
      updatePaymentMethodsDict(
        prevPaymentMethodsDict$1,
        pmName,
